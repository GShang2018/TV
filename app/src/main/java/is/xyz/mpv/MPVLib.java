...
public final class MPVLib {
    ...
    /**
     * 当前设备是否有可用的 MPV native 库。
     * 用于播放器内核切换前的可用性检测，避免选择 MPV 后播放失败再被动切换内核。
     * 判定条件：
     * 1. Android 8.0（API 26）及以上：libmpv.so 硬依赖 libvulkan.so（API 26 起才成为系统库）；
     * 2. 设备原生支持 ARM ABI：libmpv 只提供 arm64-v8a / armeabi-v7a 构建，
     *    x86/x86_64 设备即使通过 ARM 翻译装 arm64 包，libmpv 也无法可靠运行（SIGILL/SIGSEGV）。
     */
    public static boolean isSupported(Context context) {
        try {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) return false;
            boolean armDevice = false;
            ...
            if (!armDevice) return false;
            return chooseAbi(context.getApplicationContext().getAssets()) != null;
        } catch (Throwable e) {
            return false;
        }
    }
    ...
    public static synchronized boolean tryCreate(Context appctx) {
        if (contextCreationAttempted) {
            // 上次创建未完成或失败：只要上下文尚未实际创建，就允许重置后重新尝试，
            // 否则 contextCreationAttempted 一旦卡 true，后续播放永远无法再创建 MPV 上下文
            if (!contextCreated) {
        ...
        try {
            ...
            // create 抛异常时上下文未创建成功，重置标记避免永久卡死
            contextCreationAttempted = false;
            throw e;
        }
        contextCreated = true;
